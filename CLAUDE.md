...
**Text** (`Internal/TextShaper.cs`) is the heart of the library: HarfBuzz shaping, balanced wrap on shaped-cluster boundaries, shrink-to-fit (Title steps down to 70% before ellipsizing), and per-run typeface fallback for emoji/CJK. Wrapping is greedy *for line count* (`GreedyWrap` — the vertical budget shrink-to-fit/`MaxLines` rely on), then `BalanceLines` re-breaks within that fixed count via a Knuth–Plass DP that minimises summed squared raggedness (the score-based "balance" of CSS `text-wrap`), so only the horizontal break points move, never the line count or size. Overlong single words still hard-break greedily. Measurement and drawing share the same shaped glyphs — never measure text any other way. `FontResolver` defaults to the embedded Noto Sans (`Internal/EmbeddedFonts.cs` — a bundled *variable* Noto Sans plus Color Emoji/Sans JP, OFL) so output is machine-independent. `Theme.FontPath` loads one file as a single card-wide face; `Theme.FontFiles` registers files under the family name each reports, and a theme/per-element `FontFamily` resolves to a registered file *before* any system lookup (both go through one private `FileFace` helper that clones per weight along a `wght` axis, like `EmbeddedFonts.SansForWeight`). A requested family that's neither registered nor installed silently falls back to the embedded default, reported via `AshcroftDiagnostics`; fonts must not throw. Embedded faces (and the per-weight clones) are process-wide singletons — never dispose them.

**Per-element weight** (`TextStyle.Weight`, 100–900) instances the variable Noto Sans along its `wght` axis via `SKTypeface.Clone` (`EmbeddedFonts.SansForWeight`). This needs SkiaSharp ≥ 4.x (the 3.x line has no variation API); the packages are pinned to a 4.x **rc** for it. Shaping goes through `Internal/HarfBuzzShaper.cs` — an in-house HarfBuzzSharp wrapper — **not** SkiaSharp's `SKShaper`, because `SKShaper` reads only the raw font blob and ignores a typeface's variation coordinates, so a cloned variable-weight face would shape (advances/wrap) at its default weight while only the rasterized outline changed. `HarfBuzzShaper` mirrors `SKShaper` (512-em scale) and additionally pushes the face's `VariationDesignPosition` onto the HarfBuzz font. **On any SkiaSharp/HarfBuzz bump, check whether `SKShaper` now applies typeface variations itself — if it does, delete `HarfBuzzShaper` and go back to `SKShaper`.**
